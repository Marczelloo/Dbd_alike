...
public:
    ...
    // Connect gameplay systems for mesh loading in lobby
    void SetGameplaySystems(game::gameplay::GameplaySystems* gp) { m_gameplay = gp; }

    // Show "lobby is full" overlay with reason
    void ShowLobbyFullMessage(const std::string& reason) { m_lobbyFullMessage = reason; m_showLobbyFull = true; }
    void ClearLobbyFullMessage() { m_showLobbyFull = false; m_lobbyFullMessage.clear(); }
...
private:
    ...
    void DrawPlayerGlbModel(const LobbyPlayer& player);
    
    void DrawUIPanel(float x, float y, float width, float height);
    void DrawPlayerSlot(float x, float y, float width, float height, int playerIndex, bool isKillerSlot = false);
    ...
    // Lobby mesh loading for GLB models
    struct LobbyMeshEntry
    {
    ...
    std::unordered_map<std::string, LobbyMeshEntry> m_lobbyMeshCache;
    LobbyMeshEntry GetOrLoadLobbyMesh(const std::string& characterId);

    engine::ui::UiSystem* m_ui = nullptr;
    engine::render::Renderer* m_renderer = nullptr;
    engine::platform::Input* m_input = nullptr;
    game::gameplay::GameplaySystems* m_gameplay = nullptr;
    LobbySceneState m_state;
    bool m_isInLobby = false;

    // Lobby full overlay
    bool m_showLobbyFull = false;
    std::string m_lobbyFullMessage;
    bool m_loadoutMenuOpen = false;
    ...
    int m_characterDropdownStart = 0;
    ...
    int m_addonADropdownStart = 0;
    int m_addonBDropdownStart = 0;
    int m_perkDropdownStart = 0;
    ...
    static constexpr int kDropdownPageSize = 8;
